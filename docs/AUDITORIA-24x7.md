...
> **Implementados (2026-06-27):** A1/#27/#28 (serialización + timeout + keep-alive del WebSocket), A5/A9/#31 (reentrada + doble START → **409**), #49 (`-hwaccel cuda` sin `-hwaccel_output_format`), **A6/#9** (segmentos MP4/MOV en fMP4 robusto), **A8/#29/#37** (cierre del `TcpClient` NDI anterior en cada reconexión), **A11/#15** (log claro de InputType no soportado vs «sin señal»), **A7/#10** (guarda de disco con caudal AGREGADO por volumen + piso ×nº de canales). Validados en vivo: doble START → 409; stop idempotente; A6 = 4 segmentos `moof`; A7 = el «restante» de un canal cae ×2.0 al grabar 2 canales (antes ignoraba el agregado); arranque REAL + grabación OK.
>
> **Quick wins (2026-06-27):** #18 (timecode a la tasa real de la fuente), #21/#25/#26 (sin tormenta de UI: el audio ya no reconstruye el estado en cada FTPK), #22 (índices en `Sessions` — validado: los 3 en la BD viva), #24 (fuga del VM de presets), #43 (barrido de temporales `.faststart` al arrancar), #44 (remux comprueba espacio libre), #52 (preview ya no puede congelarse — `volatile`), #53 (CTS de slate sin fuga).
>
> **Robustez + auditoría (2026-06-27):** **A2/#5** (Mutex de instancia única + cuerpo de arranque en `StartHostAsync` con try/catch → fallo de arranque con diagnóstico y cierre limpio, no proceso muerto sin rastro); **#42** (nuevo `EventLogWriter`: persiste los eventos de dominio en la tabla `EventLog` en lote — auditoría 24/7; validado: `RecordingStarted/Stopped` en la BD); **#41** (caché en memoria de la lista del scheduler, invalidada en cada escritura → el tick de 1 s ya no relee SQLite 86 400 veces/día).
>
> **Medios seguros (2026-06-27):** **#20** (el auto-stop del scheduler guarda el `SessionId` que inició y solo detiene esa sesión: si el operador paró la programada y arrancó una manual, el auto-stop ya no la corta — `SchedulerService`; +1 test); **#23** (nuevo `OrphanSegmentReconciler`: al arrancar reconcilia los segmentos en disco sin registro tras un corte eléctrico en grabación segmentada, asociándolos por nombre base a la sesión hermana más reciente — núcleo PURO con 6 tests; cableado en `App.xaml.cs` tras `CloseOrphanedAsync`); **#32** (`NdiReceiver` re-detecta el formato tras una pérdida prolongada ≥5 s para que un cambio de resolución/pixel de la fuente no sirva con los parámetros viejos; la presencia se notifica DESPUÉS de re-fijar el formato para no publicar 0×0 transitorio — parcial: corrige `CurrentSignal` y cualquier reconstrucción del pipeline, no recompone en caliente un FFmpeg ya en marcha); **#33** (DeckLink: limitación documentada explícitamente — dispositivo exclusivo sin sondeo de presencia; la pérdida en caliente solo la capta el watchdog); **#17** (los canales se construyen EN PARALELO con timeout por canal y FUERA del hilo de UI — `ChannelHost` pre-resuelto en hilo de fondo antes de `app.StartAsync` + `Task.WhenAll` sobre las keys, `await` reales en vez de `.GetAwaiter().GetResult()`. Antes: construcción en SERIE bloqueando el arranque; N fuentes NDI sin señal encadenaban sus timeouts de 8 s ≈ 32 s con 4 canales). Validado: 198 unit + 31 integ verdes; arranque REAL — las 4 tuberías A–D componen en una ventana de ~70 ms (paralelo) antes de «Application started», con el reconciliador #23 corriendo sin error.
>
> **Bajos seguros (2026-06-27):** **#34** (resiliencia de servicios de fondo: `HostOptions.BackgroundServiceExceptionBehavior=Ignore` → una excepción que escape de un `BackgroundService` ya NO tumba el host 24/7, solo se loguea; complementa C1); **#36** (`_session` marcado `volatile` en `StandaloneChannelEngine`: el getter `Status` —hilo UI/API— y `OnEngineAlarm` —hilo de stderr— ven siempre el valor vigente, sin lectura obsoleta frente a Start/Stop); **#50** (guardia de reentrancia `SemaphoreSlim(1,1)`+`WaitAsync(0)` en `RefreshTodayTasks`: el timer de 30 s y los eventos del scheduler ya no solapan dos pasadas que pisen la lista HOY); **#56** (seguridad: `ValidateStreamUrl` rechaza `|`/`[`/`]` —inyección de ramas/opciones del muxer `tee`— y exige esquema whitelist en `StreamTarget.Url`; +5 tests; preventivo, streaming es Fase 2); **#57** (seguridad: el endpoint `/storage` solo admite el VOLUMEN de grabación —el del proceso— y envuelve en try/catch → no sondea rutas arbitrarias; validado en vivo: `C:\`/`C:\recordings`→200, `D:\`/UNC→400). Validado: 203 unit + 31 integ verdes; arranque REAL (4 canales LOCK) sin regresiones.
>
> **Descartados de este lote (con motivo):** **#40** (quitar el `Flush()` por frame de `D3DImagePreview` NO es seguro: `Shared`/`Staging` los gestionan dos dispositivos D3D distintos —D3D11 produce, D3D9Ex/WPF consume vía handle compartido—; `AddDirtyRect` es una pista de región, no un primitivo de sincronización entre dispositivos → arriesga tearing/frames obsoletos no verificables en remoto); **#54** (eliminar `FfmpegRecorderEngine` NO es borrado limpio: lo usan `FfmpegRecordingTests` y el `SmokeTest`; no está en el flujo de producción, así que arreglar su `StopAsync` es de bajo valor → limpieza de legado en un lote aparte); **#38** (paralelizar el bus es cambio de semántica, riesgo medio, y el aislamiento de errores por handler YA existe → diferido).
>
> **#51 verificado YA RESUELTO por #24 (2026-06-27):** inventario de TODAS las ventanas hijas (`.Show()`: PresetManager, Inputs, Schedule, GeneralSettings, ChannelSchedule; `.ShowDialog()`: RecordingName, PresetEditor, diálogos del SO) + `grep` de `+=` en todos los `*ViewModel.cs`: el ÚNICO VM de ventana que se suscribe a un singleton es `PresetManagerViewModel` (`IPresetStore.Changed`), dispuesto en `window.Closed` desde #24. Los demás VMs de ventana NO se suscriben a nada (solo llaman métodos o muestran snapshots) → sin fuga. `ChannelViewModel` (por-canal) se desuscribe correctamente en `Dispose()`. No queda defecto.
>
> **Pendientes:** A3/A4 (drift A/V NDI, lo delicado), A10 (auth, decisión de mecanismo) + Medios (#19 offset A/V NDI inicial, #30 = A3) + Bajos restantes (#35 av-sync, #39/#59 sonda NDI, #47/#48 ffmpeg-config, #55 watchdog, #58 = A10).
